require 'rails_helper'

RSpec.describe User, type: :model do
  context 'with valid attributes' do
    it 'will save' do
      expect(build(:user)).to be_valid()
    end
    it 'automatically encrypts the password into the password_digest attribute' do
      expect(build(:user).password_digest.present?).to eq(true)
    end

    it 'email is lowercase' do
      expect(build(:user, email: "NAME@MAIL.COM").email).to eq("name@mail.com")
    end
  end

  context 'with invalid attributes' do
    
    it 'name is blank' do
      expect(build(:user, name: "")).to be_invalid
    end
    it 'email is blank' do
      expect(build(:user, email: "")).to be_invalid
    end
    it 'email format is wrong' do
      invalid_formats = ["", "@", ".", "@.", "@mail.com", "asdf@mailcom", "asdfasdfcom", "asdfmail.com"]
      invalid_formats.each do |email|
        expect(build(:user, email: email)).to be_invalid
      end
    end
    it 'email is not unique' do
      create(:user)
      expect(build(:user)).to be_invalid
    end
    it 'password is blank' do
      expect(build(:user, password: "")).to be_invalid
    end
    it 'password and password_confirmation are different' do
      expect(build(:user, password_confirmation: "wrongpassword")).to be_invalid
    end

  end
end

